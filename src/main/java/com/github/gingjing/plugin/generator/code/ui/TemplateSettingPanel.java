...
import com.github.gingjing.plugin.generator.code.tool.CloneUtils;
import com.github.gingjing.plugin.generator.code.tool.ProjectUtils;
import com.github.gingjing.plugin.generator.code.ui.base.BaseGroupPanel;
import com.github.gingjing.plugin.generator.code.ui.base.BaseItemSelectPanel;
...
public class TemplateSettingPanel implements Configurable {
     ...
     * 配置信息
     */
    private CodeFlutterGenCodeConfigComponent settings;
    ...
    /** 调式面板 */
    private TemplateDebugPanel templateDebugPanel;
    ...
    TemplateSettingPanel() {
        ...
        // 配置服务实例化
        this.settings = CodeFlutterGenCodeConfigComponent.getInstance();
        // 克隆对象
    ...
    public JComponent createComponent() {
        ...
        this.baseGroupPanel = new BaseGroupPanel(new ArrayList<>(group.keySet()), this.currGroupName) {
            ...
            protected void deleteGroup(String name) {
                // 删除分组
                group.remove(name);
                currGroupName = CodeFlutterGenCodeConfigComponent.DEFAULT_NAME;
                baseGroupPanel.reset(new ArrayList<>(group.keySet()), currGroupName);
            }
            ...
            protected void changeGroup(String name) {
                ...
                if (group.get(currGroupName).getElementList().isEmpty()) {
                    // 没有元素时，需要清空编辑框
                    templateEditor.reset("empty", "");
                    templateDebugPanel.setTemplateEditor(templateEditor);
                }
            }
        };
        ...
        this.baseItemSelectPanel = new BaseItemSelectPanel<Template>(group.get(currGroupName).getElementList(), true) {
            ...
            protected void selectedItem(Template item) {
                ...
                if (templateEditor == null) {
                    ...
                    templateEditor = new TemplateEditor(project, item.getName() + PluginFileConstants.VELOCITY_FILE, item.getCode(), TEMPLATE_DESCRIPTION_INFO, velocityFileType);
                    // 代码修改回调
        ...
        // 添加调试面板
        templateDebugPanel = new TemplateDebugPanel(project, templateEditor);
        baseGroupPanel.add(templateDebugPanel.getMainPanel(), BorderLayout.EAST);
    ...
    private String findExistedGroupName(String groupName) {
        ...
        if (!group.containsKey(groupName)) {
            //尝试使用默认分组
            if (group.containsKey(CodeFlutterGenCodeConfigComponent.DEFAULT_NAME)) {
                return CodeFlutterGenCodeConfigComponent.DEFAULT_NAME;
            } else {
                //获取第一个分组
                return group.keySet().stream().findFirst().orElse(CodeFlutterGenCodeConfigComponent.DEFAULT_NAME);
            }
        }
        return groupName;
    }
