...
import com.github.gingjing.plugin.generator.code.entity.SaveFile;
...
//            processor = new RearrangeCodeProcessor(processor);
     ...
     * @param project  项目
     * @param isNeedDelete 是否需要删除
     * @since  v1.0.0
     * @date   2020年07月13日 21时49分
     */
    public void checkBlankAndDeleteFile(PsiClass psiClass, Project project, boolean isNeedDelete) {
        if (isNeedDelete) {
            String text = psiClass.getText();
            String suf = text.substring(text.indexOf("{") + 1);
            String content = org.apache.commons.lang3.StringUtils.stripEnd(suf, "}").trim();
            boolean newClass = CacheDataUtils.getInstance().isNewClass(project);
            boolean delete = false;
            if (StringUtils.isEmpty(content) && newClass) {
                String path = psiClass.getContainingFile().getVirtualFile().getPath();
                try {
                    delete = Files.deleteIfExists(Paths.get(path));
                } catch (IOException ie) {
                    ie.printStackTrace();
                }
            }
            if (delete) {
                CacheDataUtils.getInstance().setNewClass(project, false);
            }
        }
    }
}
