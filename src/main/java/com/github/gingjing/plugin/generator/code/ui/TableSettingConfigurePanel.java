...
import com.github.gingjing.plugin.generator.code.entity.ColumnConfigType;
 ...
 */
public class TableSettingConfigurePanel extends AbstractTableGroupPanel<ColumnConfigGroup, ColumnConfig> implements Configurable {
    private CodeFlutterGenCodeConfigComponent settings = CodeFlutterGenCodeConfigComponent.getInstance();

    TableSettingConfigurePanel() {
        super(CloneUtils.cloneByJson(CodeFlutterGenCodeConfigComponent.getInstance().getColumnConfigGroupMap(), new TypeReference<Map<String, ColumnConfigGroup>>() {}), CodeFlutterGenCodeConfigComponent.getInstance().getCurrColumnConfigGroupName());
    }
