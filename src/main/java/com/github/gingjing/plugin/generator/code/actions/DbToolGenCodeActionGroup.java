 ...
 */
public class DbToolGenCodeActionGroup extends ActionGroup {
    /**
     * 缓存数据工具类
     */
    ...
    private AnAction[] getMenuList() {
        String mainActionId = "com.github.gingjing.easy.code.action.generate";
        String configActionId = "com.github.gingjing.easy.code.action.config";
        ActionManager actionManager = ActionManager.getInstance();
        // 代码生成菜单
        AnAction mainAction = actionManager.getAction(mainActionId);
        if (mainAction == null) {
            mainAction = new GenCodeMainAction("Generate Code");
            actionManager.registerAction(mainActionId, mainAction);
        }
        // 表配置菜单
        AnAction configAction = actionManager.getAction(configActionId);
        if (configAction == null) {
            configAction = new DbToolTableConfigAction("Config Table");
