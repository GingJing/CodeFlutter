 ...
 */
class AutoValueProcessor extends BaseProcessor {

    private static Pattern pattern = Pattern.compile("@.*?AutoValue");
...
//            createMethod(factory, Constant.autoValueMethodTemplate.replace("$className$", classEntity.getClassName()).replace("$AdapterClassName$",
//            getAutoAdpaterClass(autoAdapter)).trim(), cls);
//        }
    }
